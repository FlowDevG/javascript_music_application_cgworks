async function loadTracks() {
  ...
  Array.from(document.getElementsByClassName('right_wrapper_top_item')).forEach((element, i) => {
    element.getElementsByClassName('top_item_image')[0].src = data[i].thumbnail;
    element.getElementsByClassName('right_wrapper_top_item_title')[0].innerHTML = data[i].title;
  });

  // Push data to Right Bottom UI
  Array.from(document.getElementsByClassName('right_wrapper_bottom_item')).forEach((element, i) => {
    element.getElementsByClassName('bottom_item_image')[0].src = data[i].thumbnail;
    element.getElementsByClassName('right_wrapper_bottom_item_desc_title')[0].innerHTML = data[i].title;
  });

  // Push data to Right Middle UI
  Array.from(document.getElementsByClassName('right_wrapper_middle_item')).forEach((element, i) => {
    element.getElementsByClassName('middle_item_image')[0].src = data[i].thumbnail;
    element.getElementsByClassName('right_wrapper_middle_item_desc_title')[0].innerHTML = data[i].title;
  });
  
  // Push data to the left menu UI
  Array.from(document.getElementsByClassName('left_wrapper_menu_bottom')).forEach((element, i) => {
    // console.log(element);
    element.getElementsByClassName('left_wrapper_menu_bottom_title').innerText = data[i].title;
  });
  ...
  const PlayAll = () => {
    Array.from(document.getElementsByClassName('top_item_button')).forEach((element) => {
  ...
  Array.from(document.getElementsByClassName('top_item_button')).forEach((element) => {
    ...
    prev.addEventListener('click', () => {
      index -= 1;
      if (index < 1) {
        index = Array.from(document.getElementsByClassName('right_wrapper_top_item')).length;
      }
    ...
    next.addEventListener('click', () => {
      index -= 0;
      index += 1;
      if (index > Array.from(document.getElementsByClassName('right_wrapper_top_item')).length) {
        index = 1;
      }
